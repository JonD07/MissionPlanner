...
Solver_Baseline::Solver_Baseline(bool heuristic) {
	if(SANITY_PRINT)
...
void Solver_Baseline::Solve(Input* input, Solution* I_crnt) {
	...
	do {
		...
		for(int l = 0; l < input->getM() && !increase_k; l++) {
			...
			for(int k : assignment_lk.at(l)) {
				...
				if(ordered_subtours.at(k).size() > 0) {
					if(DEBUG_SLVR_BL)
						printf("Improve %d:%d -- size = %ld\n", l, k, ordered_subtours.at(k).size());

					std::vector<Point> sub_tour;
					// Add the bs to start the tour
					{
						Point pt(-1, input->getX_b(), input->getY_b(), input->getZ_b());
					...
					bool valid_solution = tHeuristic.ImproveSubTour(l, input, &sub_tour, use_heuristic);
